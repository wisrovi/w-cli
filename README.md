...
| **`wpipe-steps`** | Core Extension | `Internal` | Official, production-ready modular steps for the `wpipe` engine. | Packs for DBs (Redis, MySQL, Clickhouse), SFTP, security scanner, Telegram/Slack integration. |
| **`wpipe-plugins`** | Core Extension | `Internal` | Community-driven repository for custom steps and states. | Autocataloging parser (`steps_catalog.json`), Pydantic input validation structures. |
| **`wpipe-mcp`** | Core Extension | `Internal` | Model Context Protocol (MCP) server for pipeline design. | Local stdio/SSE execution, guides LLMs to generate valid structures (DTO, states, main). |
